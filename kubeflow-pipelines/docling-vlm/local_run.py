...
TEST_CONFIGS = {
    "baseline_cpu": {
        ...
        "docling_remote_model_enabled": False,
    },
    "auto_detect": {
...
FAILURE_SCENARIOS = {
    "invalid_device": {
        "docling_accelerator_device": "Invalid",  # Invalid device
        "should_fail": True,
...
def validate_output(output_dir: Path, test_name: str) -> dict:
    ...
    if json_files:
        ...
        try:
            ...
            validation_results["checks"]["json_parsing"] = f"Failed: {e}"
    ...
    if md_files:
        ...
        try:
            ...
            validation_results["checks"]["md_parsing"] = f"Failed: {e}"
            validation_results["passed"] = False
...
def run_test_scenario(test_name: str, config: dict, should_fail: bool = False):
    ...
    try:
        ...
        if should_fail:
            ...
            # KFP wraps errors, so check if pipeline failed (which means validation worked)
